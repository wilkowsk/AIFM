...
public:
  void do_work(FarMemManager *manager) {
    ...
    auto dataframe_vector2 = manager->allocate_dataframe_vector<long long>();
    auto dataframe_vector3 = manager->allocate_dataframe_vector<long long>();
    for (uint64_t i = 0; i < kNumEntries; i++) {
      DerefScope scope;
      dataframe_vector.push_back(scope, static_cast<long long>(1337*i));
    }
    for (uint64_t i = 0; i < kNumEntries; i++) {
      DerefScope scope;
      dataframe_vector2.push_back(scope, static_cast<long long>(9001*i));
    }
    bool stop = false;
    while (!stop) {
      DerefScope scope;
      if (dataframe_vector.empty() && dataframe_vector2.empty()) {
        stop = true;
      } else if (dataframe_vector.empty()) {
        dataframe_vector3.push_back(scope, static_cast<long long>(dataframe_vector2.front(scope)));
        dataframe_vector2.pop_back(scope);
      } else if (dataframe_vector2.empty()) {
        dataframe_vector3.push_back(scope, static_cast<long long>(dataframe_vector.front(scope)));
        dataframe_vector.pop_back(scope);
      } else if (dataframe_vector.back(scope) > dataframe_vector2.back(scope)) {
        dataframe_vector3.push_back(scope, static_cast<long long>(dataframe_vector.back(scope)));
        dataframe_vector.pop_back(scope);
      } else {
        dataframe_vector3.push_back(scope, static_cast<long long>(dataframe_vector2.back(scope)));
        dataframe_vector2.pop_back(scope);
      }
    }
    for (uint64_t i = 0; i < 10000; i++) {
      DerefScope scope;
      cout << dataframe_vector3.at(scope, (2*kNumEntries) - i - 1) << endl;
    }
    uint64_t element = 0;
    while (!dataframe_vector3.empty()) {
      DerefScope scope;
      TEST_ASSERT(element <= dataframe_vector3.back(scope));
      element = dataframe_vector3.back(scope);
      dataframe_vector3.pop_back(scope);
    }
